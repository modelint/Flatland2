...
class InterpolatedBranch(Branch):
    def __init__(self, order: int, connector: TreeConnector, hanging_stems: Set[AnchoredTreeStem]):
        self.Connector = connector # overrides baseclass
        ...
        high_stems = downward_stems if downward_stems else {s for s in hanging_stems if s.Node_face == NodeFace.RIGHT}
        ...
        Branch.__init__(self, order, axis, hanging_stems)
    ...
    def Line_segment(self):
        ...
        horizontal_axis = list(self.Hanging_stems)[0].Node_face in HorizontalFace
        if horizontal_axis:
            y = self.Axis
            x1 = prev_axis if prev_axis else min({s.Anchor_position.x for s in self.Hanging_stems})
            x2 = next_axis if next_axis else max({s.Anchor_position.x for s in self.Hanging_stems})
            return Line_Segment( from_position=Position(x=x1, y=y), to_position=Position(x=x2, y=y) )
        else:
            x = self.Axis
            y1 = prev_axis if prev_axis else min({s.Anchor_position.y for s in self.Hanging_stems})
            y2 = next_axis if next_axis else max({s.Anchor_position.y for s in self.Hanging_stems})
