...
def process_leaf_stems(graft: Optional[New_Stem]) -> (Set[New_Stem], Optional[New_Stem]):
    pass
...
def layout_generalization(diagram, nodes, rnum, generalization, tree_layout):
    ...
    # Get all leaf stems in the trunk branch
    for name in lfaces.keys():
        # Current leaf stem
        lstem = New_Stem(stem_type='subclass', semantic='subclass', node=nodes[name],
                         face=lfaces[name]['face'], anchor=lfaces[name]['anchor'], stem_name=None)
        leaf_stems.add(lstem)
        # Check float status of this leaf stem
        if not floating_leaf_stem and lfaces['anchor'] == 'float':
            # There can only be one in a branch and the parser should ensure this, raising errors on duplicates
            # As a safety, we remember only the first one we iterate on
